...
package com.wie.framework.dao.hspring.handler.imp.spring;
...
//		return allCount;
	...
	public List findListOfObjBySql(final String sql, final int page, final int count) {
		return this.getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException, SQLException {
	...
	public List addbatchBySql(final String sql){
		HibernateCallback cb = new HibernateCallback() {
			public Object doInHibernate(Session session) {
				return session.createSQLQuery(sql).list();
			} // end function
