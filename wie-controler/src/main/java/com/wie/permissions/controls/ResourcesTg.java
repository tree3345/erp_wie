...
import com.wie.permissions.biz.ISystemService;
...
public class ResourcesTg extends BaseTg {
	...
	public String index(){
		try {
			String tree=this.resourcesService.getAllHTMLTrees(request);
	...
	public void save(){
		try {
			...
			re=this.resourcesService.convert(resource);
			//System.out.println("resource:"+resource.getName()+"    "+re.getName());
	...
	public void saveActions(){
		...
		try {
			...
			for(int i=0;i<strs.length;i++){
				Actions action=actionsService.findById(strs[i]);
				resSet.add(action);
			}
	...
	public void delete(){
		try {
			this.resourcesService.deleteSubResources(id+"",true);
			request.setAttribute("allTrees",this.resourcesService.getAllHTMLTrees(request));
			outJsonPlainString(response, "{\"success\":true}");
		} catch (Exception e) {
			e.printStackTrace();
