	   ...
	   //树
		function typetree()
		{
			$('#type').etree({
				...
				createUrl: '<c:url value="/intercourse/createInterType.tg"/>',
			    updateUrl: '<c:url value="/intercourse/updateInterType.tg"/>',
			    destroyUrl: '<c:url value="/intercourse/destroyInterType.tg"/>',
				dndUrl: '<c:url value="/intercourse/dndInterType.tg"/>',
		...
		{
			var realurl='<c:url value="/intercourse/getItemsInterType.tg"/>';
			if(para!=null)
			...
			$(function(){
				...
				$('#tt').datagrid({
					...
					url:realurl,
					columns:[[
						{field:'name',title:'名称',width:80,align:'right',editor:'text'},
						{field:'code',title:'编码',width:180,editor:'text'},
		                ...
		                handler : function() {
		                    if ($tt.datagrid('getChanges').length) {
		                        ...
		                        $.post("<c:url value="/intercourse/saveInterType.tg"/>", effectRow, function(data) {
		                        	    $('#tt').datagrid('reload');
		                        	    $('#type').tree('reload');
		                        		data=eval('('+data+')');
		        						if(data.success){
		...
		function delclass(){
				...
				//console.log(isLeaf);
				 ...
				 $.post('<c:url value="/intercourse/destroyJudgeInterType.tg"/>', {id:classObj.id}, function(data) {
            		data=eval('('+data+')');
            		//console.log(data);
					if(data.success){
						$('#type').etree('destroy');
