...
//---------------------------------------------------------------------
		...
		function newItem(){
			...
			actionUrl = '<c:url value="/product/saveProduct.tg"/>';
		}
		function saveItem(){
			$('#myform').form('submit', {
				url:actionUrl,
		...
		function editItem(){
			...
			if (row){
				//$('#userId').val(row.id);
				//actionUrl = '<c:url value="/product/findProduct.tg"/>';
...
//				});
				$("#dlg-buttons a::first-child").show();
				actionUrl = '<c:url value="/product/updateProduct.tg"/>';
			}else{
				alert('请选择商品修改');
			}
		}
		function delItem(){
			...
			if(row){
				$.messager.confirm('提示','确定要删除？',function(r){
					if(r){
						$('#userId').val(row.id);
						actionUrl = '<c:url value="/product/delProduct.tg"/>';
						$('#userform').form('submit', {
							url:actionUrl,
							success: function(data){
								$('#dlg').dialog('close');
